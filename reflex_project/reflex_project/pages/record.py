...
class RecordState(rx.State):
    ...
    def start_recording(self, ref: str):
        ...
        return webcam.start_recording(
            ref,
            on_data_available=RecordState.handle_video_chunk,
            on_start=RecordState.on_start_recording,
            on_stop=RecordState.on_stop_recording,
            timeslice=1000,
        )
...
def last_screenshot_widget() -> rx.Component:
    ...
    return rx.box(
        rx.cond(
            RecordState.last_screenshot,
            rx.fragment(
                rx.image(src=RecordState.last_screenshot),
                rx.text(RecordState.last_screenshot_timestamp),
            ),
            rx.center(
    ...
    return rx.vstack(
        webcam.webcam(
            ...
            on_click=webcam.upload_screenshot(
                ref=ref,
                handler=RecordState.handle_screenshot,  # type: ignore
            ),
            audio=True,
        ),
        rx.cond(
            ...
            rx.button(
                "🟢 Start Recording",
                on_click=RecordState.start_recording(ref),
                color_scheme="green",
                size="4",
            ),
            rx.button(
...
@template(route="/record", title="Record")
